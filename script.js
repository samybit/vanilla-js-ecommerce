...
function saveProduct() {
    ...
    // 2. Validate Image
    // If Creating (no ID): Image is required
    // If Editing (has ID): Image is optional (if empty, we keep old one)
    if (!id && newImage === "") {
...
function editProduct(id) {
    clearErrors(); // Clear any existing errors on the screen
...
function toggleWishlist(productId) {
    ...
    // REFRESH LOGIC:
    // If we are on the wishlist page, re-render the wishlist (so item disappears)
...
function checkout() {
    ...
    // We loop through the Cart to see what we need to deduct
    for (var i = 0; i < cart.length; i++) {
        var cartItem = cart[i];
